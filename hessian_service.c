...
static PHP_METHOD(HessianService, __construct)
{
	...
	*/
	...
	params[0] = type_map;
	call_user_function(hessian_type_map_entry, hessian_type_map, &function_name, NULL, 1, params TSRMLS_CC);
	...
	ZVAL_STRING(function_name, "__construct", 1);
	call_user_function(hessian_factory_entry, factory, &function_name, NULL, 0, params TSRMLS_CC);
	...
	call_user_function(hessian_factory_entry, factory, &function_name, NULL, 1, params TSRMLS_CC);
}
...
static PHP_METHOD(HessianService, registerObject)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "z", &service, &arg_options) == FAILURE) {
		RETURN_FALSE;
	}
	...
	*/
	if (Z_TYPE_P(service) == IS_OBJECT){
		zend_get_object_classname(service, &name, &name_len TSRMLS_CC);
	...
	*/
	...
	if (Z_TYPE_P(service) == IS_STRING){
		...
		cls_service = zend_fetch_class(name, Z_STRLEN_P(name), 0 TSRMLS_DC);
		ALLOC_ZVAL(service);
		object_init_ex(service, cls_service);
		end_update_property(hessian_service_entry, self, ZEND_STRL("service"), service TSRMLS_DC);
	}
	...
	*/
	...
	if (IS_OBJECT != Z_TYPE_P(service)){
		...
		cls_service = zend_fetch_class("Exception", strlen("Exception")-1, 0 TSRMLS_DC);
		zend_throw_exception(cls_service, "Error registering service object",8 TSRMLS_DC);
		return;
	}
...
static PHP_METHOD(HessianService, writeFault)
{
	...
	ZVAL_STRING(header, "HTTP/1.1 500 Hessian service error");
	sapi_header_op(SAPI_HEADER_REPLACE:SAPI_HEADER_ADD, &header TSRMLS_CC);
...
static PHP_METHOD(HessianService, handle)
{
	...
	*/
	...
	if (Z_TYPE_P(service) != IS_OBJECT){
		zval header, str;
		ZVAL_STRING(&header, "HTTP/1.1 500 Hessian not configured");
		sapi_header_op(SAPI_HEADER_REPLACE:SAPI_HEADER_ADD, &header TSRMLS_CC);
		ZVAL_STRING(str, "Serviced object not registered!");
	...
	*/
	...
	ZVAL_STRING(varname, "_SERVER");
	hash_value = Z_HASH_P(varname);
	zend_hash_quick_find(EG(symbol_table), Z_STRVAL_P(varname), Z_STRLEN_P(varname)+1, hash_value, (void **) &server);
	zend_hash_find(Z_ARRVAL_P(server), "REQUEST_METHOD", strlen("REQUEST_METHOD")-1, request_method);
	if (strcmp(Z_STRVAL_P(request_method), "POST") == 0){
		...
		if (i_zend_is_true(display_info)){
			...
			ZVAL_STRING(&header, "HTTP/1.1 500 Hessian Requires POST");
			sapi_header_op(SAPI_HEADER_REPLACE:SAPI_HEADER_ADD, &header TSRMLS_CC);
			ZVAL_STRING(error_str, "<h1>Hessian Requires POST</h1>");
	...
	ZVAL_STRING(error_handle, "hessianServiceErrorHandler");
	...
	*/
	...
	object_init_ex(in_stream, hessian_buffered_stream_entity);
	ZVAL_STRING(function_name, "__construct", 1);
	params[0] = ph;
	...
	*/
	...
	params[0] = writer;
	params[1] = options;
	call_user_function(NULL, parser, &factory, writer, 2, params TSRMLS_DC);
	ZVAL_STRING(function_name, "setTypeMap", 1);
	params[0] = type_map;
	...
	*/
	...
	if (Z_STRLEN_P(std_output) > 0 && !i_zend_is_true(ignore_output)){
		...
		ZVAL_STRING(function_name, "__construct", 1);
		ZVAL_STRING(param1, "Unclean output detected");
		ZVAL_LONG(param2, 0);
		params[0] = param1;
		params[1] = param2;
	...
	*/
	...
	ZVAL_STRING(&header1, "Content-type: application/binary");
	sapi_header_op(SAPI_HEADER_REPLACE:SAPI_HEADER_ADD, &header1 TSRMLS_CC);
	ZVAL_STRING(&header2, "Connection: close");
	sapi_header_op(SAPI_HEADER_REPLACE:SAPI_HEADER_ADD, &header2 TSRMLS_CC);
...
static PHP_METHOD(HessianService, callMethod)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "zz", &method, &params) == FAILURE) {
		RETURN_FALSE;
	}
	...
	*/
	...
	if (i_zend_is_true(callable)){
		...
		call_user_function_helper(fun, params, ret_val, EG(function_table));
		FREE_ZVAL(fun);
	}else{
